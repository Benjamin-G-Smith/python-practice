...
def error_rate_by_service(entries: list[dict]) -> dict[str, float]:
    ...
    return {
        service: round(
            sum(1 for entry in service_entries if entry["level"] == "ERROR") / len(service_entries),
            2,
        )
        for service, service_entries in groups.items()
    }
...
def top_error_services(entries: list[dict], n: int) -> list[str]:
    ...
    groups = group_by_service(entries)
    error_counts = {
        service: sum(1 for entry in service_entries if entry["level"] == "ERROR")
        for service, service_entries in groups.items()
    }
    eligible = [service for service, count in error_counts.items() if count > 0]
    eligible.sort(key=lambda service: (-error_counts[service], service))
    return eligible[:n]
